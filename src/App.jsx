...
// Grupos de despesa para cálculo dos indicadores
...
function Dashboard({usuario}) {
  const [contas,setC]=useState([]); const [envios,setE]=useState([]); const [vendas,setV]=useState([]); const [load,setL]=useState(true);
  ...
  function calcLoja(lojaId) {
    ...
    // CMV = envios producao + contas do grupo CMV
    ...
    // Detalhes
    const detCMV={};
    cL.filter(c=>SUBS_CMV.some(s=>c.grupo?.toUpperCase().includes(s))).forEach(c=>{const k=c.tipo||c.grupo||"Outros"; detCMV[k]=(detCMV[k]||0)+Number(c.valor);});
    if(cmvEnvios>0) detCMV["Proteínas (Produção)"]=(detCMV["Proteínas (Produção)"]||0)+cmvEnvios;
    const detCMO={};
    cL.filter(c=>SUBS_CMO.some(s=>c.grupo?.toUpperCase().includes(s))).forEach(c=>{const k=c.tipo||c.grupo||"Outros"; detCMO[k]=(detCMO[k]||0)+Number(c.valor);});
    ...
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:24,flexWrap:"wrap",gap:12}}>
        <div>
          ...
          <div style={{fontSize:13,color:"#5A6070",marginTop:4}}>{lojaFiltro?LOJAS.find(l=>l.id===lojaFiltro)?.nome:"Todas as lojas"} · Base: Contas a Pagar</div>
        </div>
        ...
        {d.fat===0&&d.cmvVal===0&&(
          <div style={{...S.card,marginBottom:24,borderColor:"#F39C12",background:"#1A1500"}}>
            <div style={{fontSize:13,color:"#F39C12"}}>
              ⚠️ Nenhum dado encontrado para este período. Certifique-se de que as <strong>Contas a Pagar</strong> estão lançadas com a loja correta e vencimento dentro do mês selecionado, e que as <strong>Vendas</strong> estão registradas no Módulo Financeiro.
            </div>
          </div>
        )}
...
function Producao({usuario}) {
  ...
  const [form,setForm]=useState({loja_id:"",data:hoje(),produto_id:""});
  ...
  useEffect(()=>{ if(aba==="registrar"||aba==="visualizar") carregarEnvios(); },[filtro,aba]);
  ...
  async function carregarEnvios(){
    ...
    let q=supabase.from("envios_producao").select("*, produtos_producao(nome,gramatura,matriz_precos(item))").gte("data",ini).lte("data",fim).order("data",{ascending:false});
  ...
  const precoPorcao=prodSel ? (prodSel.gramatura/1000)*Number(prodSel.matriz_precos?.valor||0) : 0;

  async function salvar(){
    if(!form.loja_id||!form.produto_id)return; setSaving(true);
    await supabase.from("envios_producao").insert({
      loja_id:form.loja_id, data:form.data,
      produto_id:Number(form.produto_id),
      item:prodSel?.nome||"",
      gramatura:prodSel?.gramatura||0,
      proteina:prodSel?.matriz_precos?.item||"",
      quantidade:1,
      valor_unitario:precoPorcao,
      usuario_id:usuario.id,
    });
    setForm(f=>({...f,produto_id:""})); carregarEnvios(); setSaving(false);
  }

  async function excluir(id){ await supabase.from("envios_producao").delete().eq("id",id); carregarEnvios();}

  // Agrupamento para visualizador
  const resumoPorLoja = LOJAS.filter(l=>l.id!=="producao").map(loja=>{
    ...
    const porcoes=eL.length;
    const gramas=eL.reduce((s,e)=>s+Number(e.gramatura||0),0);
    const porProduto={};
    eL.forEach(e=>{ const k=e.item||"?"; if(!porProduto[k]) porProduto[k]={qtd:0,gramas:0,valor:0}; porProduto[k].qtd+=1; porProduto[k].gramas+=Number(e.gramatura||0); porProduto[k].valor+=Number(e.valor_total||e.valor_unitario||0); });
    return {...loja,total,porcoes,gramas,porProduto,eL};
  }).filter(l=>l.porcoes>0);
    ...
    <div>
      ...
      {aba==="registrar"&&(
        ...
        <div style={{...S.card,marginBottom:24}}>
          <div style={S.cT}>Registrar Envio de Porção</div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 2fr auto",gap:12,alignItems:"end"}}>
            ...
            <div><label style={S.lbl}>Data</label>
              <input style={S.inp} type="date" value={form.data} onChange={e=>setForm(f=>({...f,data:e.target.value}))}/>
            </div>
            <div><label style={S.lbl}>Produto / Porção</label>
              <select style={S.sel} value={form.produto_id} onChange={e=>setForm(f=>({...f,produto_id:e.target.value}))}>
                <option value="">Selecione…</option>
                {produtos.map(p=>(
                  <option key={p.id} value={p.id}>
                    {p.nome} — {p.gramatura}g — {fmt((p.gramatura/1000)*Number(p.matriz_precos?.valor||0))}
                  </option>
                ))}
              </select>
            </div>
          ...
          {prodSel&&(
            <div style={{marginTop:12,display:"flex",gap:16,fontSize:13,color:"#8090A8"}}>
              <span>Proteína: <strong style={{color:"#FF6B35"}}>{prodSel.matriz_precos?.item}</strong></span>
              <span>Gramatura: <strong>{prodSel.gramatura}g</strong></span>
              <span>Preço/kg: <strong>{fmt(prodSel.matriz_precos?.valor)}</strong></span>
              <span>Valor da porção: <strong style={{color:"#FF6B35"}}>{fmt(precoPorcao)}</strong></span>
            </div>
          )}
        </div>
        ...
        <div style={S.card}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
            <div style={S.cT}>Envios do Dia</div>
            <div style={{display:"flex",gap:8}}>
              <input style={{...S.inp,width:150}} type="date" value={filtro.mes.length===7?hoje():filtro.data||hoje()} onChange={e=>setFiltro(f=>({...f,data:e.target.value,mes:e.target.value.slice(0,7)}))}/>
          ...
          <table style={S.tbl}>
            <thead><tr>{["Loja","Produto","Proteína","Gramatura","Valor Porção",""].map(h=><th key={h} style={S.th}>{h}</th>)}</tr></thead>
            <tbody>
              {envios.length===0
                ?<tr><td colSpan={6} style={{...S.td,color:"#5A6070",textAlign:"center"}}>Nenhum envio registrado</td></tr>
                :envios.map(e=>{const l=LOJAS.find(x=>x.id===e.loja_id); return(
                  <tr key={e.id}>
                    ...
                    <td style={S.td}>{e.proteina||e.produtos_producao?.matriz_precos?.item||"—"}</td>
                    <td style={S.td}>{e.gramatura||e.produtos_producao?.gramatura||"—"}g</td>
                    <td style={{...S.td,fontWeight:700}}>{fmt(Number(e.valor_total||e.valor_unitario||0))}</td>
      ...
      {aba==="visualizar"&&(
        ...
        {resumoPorLoja.length===0
          ?<div style={{...S.card,color:"#5A6070"}}>Nenhum envio registrado para este período.</div>
...
function Financeiro({usuario}) {
  ...
  const [valV,onValV,numV,setValV]=useMoeda(); const [valD,onValD,numD,setValD]=useMoeda();
...
function ModalVerConta({conta, onClose}) {
  ...
  async function conferirComIA() {
    ...
    try {
      ...
      const resp = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          messages:[{
            role:"user",
            content:[
              { type:"image", source:{ type:"url", url:conta.comprovante_url } },
              { type:"text", text:`Este é um cupom fiscal ou recibo. O valor lançado no sistema é R$ ${valorLancado.toFixed(2).replace(".",",")}.\n\n1. Identifique o fornecedor/estabelecimento\n2. Identifique a data do documento\n3. Some todos os itens e encontre o valor TOTAL\n4. Compare com o valor lançado R$ ${valorLancado.toFixed(2).replace(".",",")}\n5. Informe se há DIVERGÊNCIA ou CONFIRMADO\n\nResponda APENAS em JSON: {"fornecedor":"...","data":"...","total_documento":"R$ X,XX","valor_lancado":"R$ X,XX","status":"CONFIRMADO ou DIVERGÊNCIA","observacao":"..."}` }
            ]
          }]
        })
      });
      ...
      try { setIaResult(JSON.parse(clean)); }
      catch { setIaResult({status:"ERRO", observacao:txt.slice(0,300)}); }
    } catch(e) {
      setIaResult({status:"ERRO", observacao:"Não foi possível analisar o documento."});
    }
    setIaLoad(false);
  }

  const campos = [
    {label:"Fornecedor",      val:conta.fornecedor},
    {label:"Loja",            val:conta.loja_id},
    {label:"Grupo",           val:conta.grupo},
    {label:"Subgrupo / Tipo", val:conta.tipo},
    {label:"Valor",           val:fmt(Number(conta.valor))},
    {label:"Vencimento",      val:fmtData(conta.vencimento)},
    {label:"Data de Chegada", val:fmtData(conta.data_chegada)},
    {label:"Forma Pagamento", val:conta.forma_pagamento},
    {label:"Nº NF / Boleto",  val:conta.numero_nf},
    {label:"Conferido por",   val:conta.conferido_por},
    {label:"Inserido por",    val:conta.inserido_por},
    {label:"Data Pagamento",  val:fmtData(conta.data_pagamento)},
    {label:"Banco",           val:conta.banco},
  ];
    ...
    <div style={S.overlay}>
      <div style={{...S.card, width:620, maxHeight:"90vh", overflowY:"auto", borderColor:cor}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
          <div>
            <div style={{fontSize:16,fontWeight:700}}>{conta.fornecedor}</div>
            <span style={S.badge(cor)}>{status}</span>
          </div>
          <button style={{...S.btn("ghost"),fontSize:20,lineHeight:1}} onClick={onClose}>✕</button>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:16}}>
          {campos.map(c=>c.val?(
            <div key={c.label} style={{background:"#0F1117",borderRadius:8,padding:"10px 12px"}}>
              <div style={{fontSize:11,color:"#5A6070",marginBottom:2}}>{c.label}</div>
        ...
        {conta.chave_pix&&(
          <div style={{background:"#0F1117",borderRadius:8,padding:12,marginBottom:12}}>
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <span style={{fontFamily:"monospace",fontSize:13}}>{conta.chave_pix}</span>
              <button style={{...S.btn("primary"),padding:"4px 10px",fontSize:11}} onClick={()=>navigator.clipboard.writeText(conta.chave_pix)}>📋 Copiar</button>
            </div>
          </div>
        )}
        {conta.codigo_barras&&(
          <div style={{background:"#0F1117",borderRadius:8,padding:12,marginBottom:12}}>
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:8}}>
              <span style={{fontFamily:"monospace",fontSize:11,wordBreak:"break-all"}}>{conta.codigo_barras}</span>
              <button style={{...S.btn("primary"),padding:"4px 10px",fontSize:11,whiteSpace:"nowrap"}} onClick={()=>navigator.clipboard.writeText(conta.codigo_barras)}>📋 Copiar</button>
            </div>
          </div>
        )}
        {/* Cupom/Recibo com IA ANTES do pagamento */}
            ...
            {isImg(conta.comprovante_url)?(
              <div>
                ...
                <button style={{...S.btn("primary"),width:"100%",marginTop:8}} onClick={conferirComIA} disabled={iaLoad}>
                  {iaLoad?"🤖 Analisando o documento…":"🤖 Conferir valor com IA"}
                </button>
              </div>
            ):(
              <a href={conta.comprovante_url} target="_blank" rel="noreferrer"
                style={{display:"block",background:"#0F1117",borderRadius:8,padding:12,color:"#3A8FE8",fontSize:13,textDecoration:"none"}}>
        ...
        {iaResult&&(
          <div style={{background:"#0F1117",borderRadius:8,padding:16,marginBottom:16,border:`2px solid ${iaResult.status==="CONFIRMADO"?"#2ECC71":iaResult.status==="DIVERGÊNCIA"?"#E74C3C":"#F39C12"}`}}>
            <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:12}}>
              ...
              <span style={{fontWeight:700,fontSize:16,color:iaResult.status==="CONFIRMADO"?"#2ECC71":iaResult.status==="DIVERGÊNCIA"?"#E74C3C":"#F39C12"}}>
                {iaResult.status==="CONFIRMADO"?"Valor Confirmado pela IA":iaResult.status==="DIVERGÊNCIA"?"Divergência Detectada":"Não foi possível analisar"}
              </span>
            </div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:8}}>
              ...
              {iaResult.data&&<div style={{fontSize:12}}><span style={{color:"#5A6070"}}>Data doc.: </span><strong>{iaResult.data}</strong></div>}
              {iaResult.total_documento&&<div style={{fontSize:12}}><span style={{color:"#5A6070"}}>Total no documento: </span><strong style={{color:iaResult.status==="DIVERGÊNCIA"?"#E74C3C":"#2ECC71"}}>{iaResult.total_documento}</strong></div>}
    ...
    <div style={S.overlay}>
      <div style={{...S.card,width:480,borderColor:problema?"#9B59B6":"#2ECC71"}}>
        ...
        {!problema&&<>
          ...
          <div style={{marginBottom:16}}><label style={S.lbl}>📎 Comprovante de Pagamento</label><input style={{...S.inp,cursor:"pointer"}} type="file" accept=".pdf,.png,.jpg,.jpeg" onChange={e=>setArq(e.target.files[0])}/></div>
        </>}
        {problema&&<div style={{background:"#1A0A1A",border:"1px solid #9B59B6",borderRadius:8,padding:12,marginBottom:16,fontSize:13,color:"#C8A0E8"}}>O status será marcado como <strong>PROBLEMA NO PAGAMENTO</strong>. O financeiro poderá editar e corrigir as informações.</div>}
    ...
    <div style={{...S.card,marginBottom:24,borderColor:editId?"#3A8FE8":"#FF6B35"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:12,marginBottom:12}}>
        ...
        <div style={{display:"flex",gap:8,alignItems:"end"}}>
          <button style={{...S.btn("primary"),flex:1}} onClick={onSalvar} disabled={sv}>{sv?"Salvando…":editId?"Salvar Edição":"Salvar"}</button>
      ...
      {form.forma_pagamento==="BOLETO"&&<div style={{marginTop:4}}><label style={S.lbl}>📊 Código de Barras</label><input style={{...S.inp,fontFamily:"monospace"}} placeholder="Cole ou use o leitor de código de barras" value={form.codigo_barras} onChange={e=>setForm(f=>({...f,codigo_barras:e.target.value}))}/></div>}
    </div>
  );
}
...
function Contas({usuario}) {
  ...
  const [arqUpload,setArqUpload]=useState(null);
  ...
  async function carregar(){
    ...
    const{data}=await q;
    let r=data||[];
    if(fSt) r=r.filter(c=>calcStatus(c)===fSt);
  ...
  return (
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:20}}>
        <div><h2 style={{fontSize:20,fontWeight:700,margin:0}}>Contas a Pagar</h2><div style={{fontSize:13,color:"#5A6070",marginTop:4}}>Organograma de Pagamentos · {contas.length} registros</div></div>
      ...
      <div style={S.card}>
        <div style={{display:"flex",gap:8,marginBottom:16,flexWrap:"wrap"}}>
          ...
          <div style={{marginLeft:"auto",fontSize:12,color:"#5A6070",display:"flex",alignItems:"center"}}>
                ...
                <tr key={c.id}>
                  ...
                  <td style={S.td}>
                    <div style={{display:"flex",gap:4,alignItems:"center"}}>
                      ...
                      {(st==="A VENCER"||st==="EM ATRASO"||st==="HOJE"||st==="PROBLEMA NO PAGAMENTO")&&usuario.perfil==="admin"&&(
                        <button style={{...S.btn("primary"),padding:"4px 8px",fontSize:11}} onClick={()=>setPId(c.id)}>Pagar</button>
                      )}
          ...
          {totalPaginas>1&&(
            <div style={{display:"flex",justifyContent:"center",gap:8,marginTop:16}}>
              ...
              {Array.from({length:totalPaginas},(_,i)=>i+1).map(p=>(
                <button key={p} style={{...S.btn(p===pagina?"primary":"secondary"),padding:"8px 12px"}} onClick={()=>setPagina(p)}>{p}</button>
              ))}
...
function Cadastros({usuario}) {
  ...
  const [editProd,setEP]=useState(null);
  const [editPreco,setEPR]=useState(null);
  ...
  async function carregarProdutos(){ const{data}=await supabase.from("produtos_producao").select("*, matriz_precos(item,valor)").eq("ativo",true).order("nome"); setProdutos(data||[]); }
  ...
  async function addProduto(){
    if(!novoProd.nome||!novoProd.gramatura||!novoProd.proteina_id)return; setSv(true);
    await supabase.from("produtos_producao").insert({nome:novoProd.nome.toUpperCase(),gramatura:Number(novoProd.gramatura),proteina_id:Number(novoProd.proteina_id)});
    setNP({nome:"",gramatura:"",proteina_id:""}); carregarProdutos(); setSv(false);
  }
  async function salvarEdicaoProduto(){
    if(!editProd)return; setSv(true);
    await supabase.from("produtos_producao").update({nome:editProd.nome.toUpperCase(),gramatura:Number(editProd.gramatura),proteina_id:Number(editProd.proteina_id)}).eq("id",editProd.id);
    setEP(null); carregarProdutos(); setSv(false);
  }
  async function delProduto(id){ await supabase.from("produtos_producao").update({ativo:false}).eq("id",id); carregarProdutos(); }

  async function salvarPreco(){
    if(!editPreco)return; setSv(true);
    await supabase.from("matriz_precos").update({valor:Number(editPreco.valor)}).eq("id",editPreco.id);
    setEPR(null); carregarPrecos(); carregarProdutos(); setSv(false);
  }
  ...
  return (
    <div>
        ...
        <div>
          <div style={{...S.card,marginBottom:24}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"2fr 1fr 2fr auto",gap:12,alignItems:"end"}}>
              ...
              <div><label style={S.lbl}>Proteína (Matriz de Preços)</label>
                <select style={S.sel} value={novoProd.proteina_id} onChange={e=>setNP(f=>({...f,proteina_id:e.target.value}))}>
                  <option value="">Selecione a proteína…</option>
                  {precos.map(p=><option key={p.id} value={p.id}>{p.item} — {fmt(p.valor)}/{p.unidade}</option>)}
                </select>
              </div>
              <button style={S.btn("primary")} onClick={addProduto} disabled={sv}>+ Adicionar</button>
            </div>
            {novoProd.gramatura&&novoProd.proteina_id&&(
              <div style={{marginTop:10,fontSize:13,color:"#8090A8"}}>
                Preço estimado da porção: <strong style={{color:"#FF6B35"}}>{fmt((Number(novoProd.gramatura)/1000)*Number(precos.find(p=>p.id===Number(novoProd.proteina_id))?.valor||0))}</strong>
              </div>
            )}
          </div>

          {editProd&&(
            <div style={{...S.card,marginBottom:24,borderColor:"#3A8FE8"}}>
              <div style={S.cT}>✏️ Editar Produto</div>
              <div style={{display:"grid",gridTemplateColumns:"2fr 1fr 2fr auto",gap:12,alignItems:"end"}}>
                <div><label style={S.lbl}>Nome</label><input style={S.inp} value={editProd.nome} onChange={e=>setEP(f=>({...f,nome:e.target.value}))}/></div>
                <div><label style={S.lbl}>Gramatura (g)</label><input style={S.inp} type="number" value={editProd.gramatura} onChange={e=>setEP(f=>({...f,gramatura:e.target.value}))}/></div>
                <div><label style={S.lbl}>Proteína</label>
                  <select style={S.sel} value={editProd.proteina_id} onChange={e=>setEP(f=>({...f,proteina_id:e.target.value}))}>
                    {precos.map(p=><option key={p.id} value={p.id}>{p.item} — {fmt(p.valor)}/{p.unidade}</option>)}
                  </select>
                </div>
                <div style={{display:"flex",gap:8}}>
                  <button style={S.btn("primary")} onClick={salvarEdicaoProduto} disabled={sv}>Salvar</button>
                  <button style={S.btn("secondary")} onClick={()=>setEP(null)}>Cancelar</button>
                </div>
              </div>
            </div>
          )}
          ...
          <div style={S.card}>
            ...
            <table style={S.tbl}>
              <thead><tr>{["Produto","Gramatura","Proteína","Preço/kg","Preço Porção",""].map(h=><th key={h} style={S.th}>{h}</th>)}</tr></thead>
              <tbody>
                {produtos.map(p=>{
                  ...
                  return(
                    <tr key={p.id}>
                      ...
                      <td style={S.td}>{p.gramatura}g</td>
                      <td style={S.td}>{p.matriz_precos?.item||"—"}</td>
                      <td style={S.td}>{fmt(p.matriz_precos?.valor)}/kg</td>
                      <td style={{...S.td,fontWeight:700,color:"#FF6B35"}}>{fmt(preco)}</td>
                      <td style={S.td}>
                        <div style={{display:"flex",gap:4}}>
                          <button style={{...S.btn("secondary"),padding:"4px 8px",fontSize:11}} onClick={()=>setEP({...p,proteina_id:p.proteina_id})}>✏️</button>
                          <button style={{...S.btn("danger"),padding:"4px 8px",fontSize:11}} onClick={()=>delProduto(p.id)}>🗑️</button>
                        </div>
                      </td>
                    </tr>
                  );
                })}
                {produtos.length===0&&<tr><td colSpan={6} style={{...S.td,color:"#5A6070",textAlign:"center"}}>Nenhum produto cadastrado</td></tr>}
              </tbody>
            </table>
          </div>
        </div>
      ...
      {aba==="precos"&&(
        <div style={S.card}>
          <div style={S.cT}>Matriz de Preços — Editar valores por kg/unidade</div>
          {editPreco&&(
            <div style={{background:"#0F1117",borderRadius:8,padding:12,marginBottom:16,display:"flex",gap:12,alignItems:"end"}}>
              <div style={{flex:1}}><label style={S.lbl}>Item: {editPreco.item}</label><div style={{fontSize:13,color:"#5A6070"}}>Novo valor (R$/{editPreco.unidade})</div></div>
              <div style={{width:160}}><input style={S.inp} type="number" step="0.01" value={editPreco.valor} onChange={e=>setEPR(f=>({...f,valor:e.target.value}))}/></div>
              <button style={S.btn("primary")} onClick={salvarPreco} disabled={sv}>Salvar</button>
              <button style={S.btn("secondary")} onClick={()=>setEPR(null)}>Cancelar</button>
            </div>
          )}
          <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,maxHeight:500,overflowY:"auto"}}>
            {precos.map(p=>(
              <div key={p.id} style={{background:"#0F1117",borderRadius:8,padding:"10px 12px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <div>
                  <div style={{fontSize:12,color:"#C8D0DC"}}>{p.item}</div>
                  <div style={{fontSize:14,fontWeight:700,color:"#FF6B35"}}>{fmt(p.valor)}<span style={{fontSize:11,color:"#5A6070"}}>/{p.unidade}</span></div>
                </div>
                <button style={{...S.btn("secondary"),padding:"4px 8px",fontSize:11}} onClick={()=>setEPR({...p})}>✏️</button>
              </div>
            ))}
          </div>
        </div>
